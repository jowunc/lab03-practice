...
typedef struct {
 ...
 unsigned char allocated; // VPN is reserved for an active allocation
 unsigned char swapped; // page is on the swap
 int swap_slot; // -1 if not on swap, otherwise the index of the swap slot
} page_t;
...
typedef struct {
 ...
 // swap table, each slot -> vpn, -1 if empty
 int* swap_table;
...
// additional functions for honors section


/*
* Allocates a contiguous block of frames in physical memory, using swap space if necessary.
*
* Parameters:
*   size - The number of contiguous frames required.
*
...
*   The starting frame index (physical address) of the allocated block
*   if successful.
*   Returns FRAME_ERROR if allocation fails.
*/
int allocate_with_swap( unsigned int size );


/*
 * Enqueues a virtual page number (VPN) into the FIFO queue for page replacement.
 *
 * Parameters:
 *   vpn - The virtual page number to be added to the FIFO queue.
 *
 * Returns:
 *  None.
 */
...
/*
* Pop a virtual page number (VPN) from the FIFO queue for page replacement.
*
* Parameters:
*   None.
*
* Returns:
*   The virtual page number removed from the FIFO queue.
*   Returns VPN_ERROR if the queue is empty.
*/
int fifo_pop();
...
/*
* Evicts one page from memory using the FIFO page replacement algorithm and swaps it out.
*
* Parameters:
*  None.
*
* Returns:
*  1 if a page was successfully evicted and swapped out.
*  0 if no page could be evicted (when FIFO queue is empty or no swap slot available).
*/
int alloc_swap_slot();
...
/*
* Removes stale entries from the FIFO replacement queue.
*
* Parameters:
*  None.
*
